...
def fileClick(clicked):
    def choose_file():
        ...
        if file_path:
            ...
            input_image_label.image = photo # Keep a reference to the photo to avoid garbage collection
    return choose_file
...
def process(clicked, captioner, classifier, input_image_label, output_label,option_var):
    ...
    if input_image_label.image!=NONE:
        ...
        pil_image = pil_image.resize((500, 500)) # Resize the image to fit the input size of the models
...
if __name__ == '__main__':
    ...
    frame2=Frame(root,borderwidth=6,bg="black",relief=SUNKEN)
    ...
    file_button = Button(frame,padx=30, pady=5,highlightbackground="white",bg="black",fg="white", text="OPEN",font="bold", command=fileClick(clicked="file_button"))
    file_button.pack(side=RIGHT, anchor=N)
    my_font = font.Font(family="Helvetica", size=15, weight="bold")
    output_label = Label(frame2)
    output_label = Listbox(frame2,width=70, height=8,font=my_font)
    output_label.pack()
    output_label.pack()
    
    option_var = StringVar()
    option_var.set("IMAGE CAPTIONING")
    option_menu = OptionMenu(frame,option_var, "IMAGE CAPTIONING", "IMAGE CLASSIFICATION")
    option_menu.configure(background="black",highlightbackground="black",foreground="white",padx="30",pady="5",font="bold")
    option_menu.pack(side=RIGHT, anchor=N)

    # Declare the process button
 
    process_button = Button(frame,highlightbackground="white",padx=30, pady=5,bg="black",fg="white", text="PROCESS",font="bold", command=partial(process, clicked="process_button", captioner=captioner, classifier=classifier, input_image_label=input_image_label, output_label=output_label, option_var=option_var))
